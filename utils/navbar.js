$(document).ready(function() {
    $("#search-bar").autocomplete({
        ...
        source: async function(req, res) {
            let data = await fetch(`https://ecommerce.hadikaraki.net/autocomplete?name=${$("#search-bar").val()}&option=${$("#option").val()}`)
...
$(document).ready(function() {
    $("#search-bar-mobile").autocomplete({
        source: async function(req, res) {
            let data = await fetch(`https://ecommerce.hadikaraki.net/autocomplete?name=${$("#search-bar-mobile").val()}&option=${$("#option2").val()}`)
                .then(results => results.json())
                .then(results => results.map(result => {
                    return {
                        label: result.name,
